...
def run_env_loop(
    ...
    while True:  # For each episode.
        ...
        while True:  # For each step in the current episode.
            ...
            a_tm1 = a_t
            # TODO: Understand, why here we have only 4 params return!
            # observation, reward, done, info = env.step(a_tm1)
            observation, reward, terminated, truncated, info = env.step(a_tm1)
...
def run_env_steps(num_steps: int, agent: types_lib.Agent, env: gym.Env, trackers: Iterable[Any]) -> Mapping[Text, float]:
    ...
    """
    logging.info(f'run_env_steps env: {env}')
    
    seq = run_env_loop(agent, env)
    
    logging.info(f'run_env_steps after run_env_loop')
...
def run_parallel_training_iterations(
    ...
    """
    print ("In run_parallel_training_iterations")
    ...
    print ("run_parallel_training_iterations learner.start()")
    learner.start()
    ...
    print ("run_parallel_training_iterations logger.start()")
    logger.start()
    ...
    for actor, actor_env, tb_log_prefix in zip(actors, actor_envs, actor_tb_log_prefixes):
        ...
        print ("run_parallel_training_iterations p.start()")
        p.start()
        processes.append(p)
    ...
    print ("run_parallel_training_iterations logger.join()")
    # learner.join()
    logger.join()
    ...
    print("run_parallel_training_iterations finished")
    logging.info("run_parallel_training_iterations finished")
...
def run_actor(
    ...
    data_queue: multiprocessing.Queue,
    log_queue: multiprocessing.SimpleQueue,
    ui_log_queue: multiprocessing.SimpleQueue,
    num_train_steps: int,
    iteration_count: multiprocessing.Value,
    start_iteration_event: multiprocessing.Event,
    stop_event: multiprocessing.Event,
    ...
    * The actor whoever finished the current iteration first will reset `start_iteration_event` to False,
    ...
    while not stop_event.is_set():
        if global_stop_event.is_set():
            logging.info('Stopped by user in run_actor from UI')
            break
        ...
        # Add training statistics to UI queue, so the UI statistics process can display current and max values.
        ui_log_queue.put(log_output)
...
def run_learner(
    ...
    Args:
        ...
        log_queue: a multiprocessing.SimpleQueue proxy, used send evaluation statistics to logger.
        ui_log_queue: a multiprocessing.Queue used send evaluation statistics to UI.
    ...
    for iteration in range(1, num_iterations + 1):        
        if global_stop_event.is_set():
            logging.info('Stopped globally by user from main UI')
            break
        ...
        run_learner_loop(learner, data_queue, num_actors, learner_trackers, global_stop_event)
...
def run_learner_loop(
    ...
    while True:
        ...
        try:
            if global_stop_event.is_set():
                logging.info('Stopped by user in run_learner_loop')
                break
            item = data_queue.get()
